...
function fetchparfumpictures() {
    const url = "http://localhost:1337/api/parfumpictures?populate=*";
    ...
    fetch(url, {
        ...
        .then(data => {
            const parfumpictures = data.data; 
            console.log(parfumpictures)           // Clear content
            slides.innerHTML = ""; // Clear previous content

            // Create and display images for each parfumpicture
            parfumpictures.forEach(parfumpicture => {
                const image = document.createElement("img");
                image.classList.add("slide");
                image.src = `http://localhost:1337${parfumpicture.attributes.photo.data.attributes.url}`;
...
function fetchProducts() {
    const url = "http://localhost:1337/api/parfumproducts?populate=*";
    const productContainer = document.querySelector(".parfumproductsection"); // Corrected selector
