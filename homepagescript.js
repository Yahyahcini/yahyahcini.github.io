...
function fetchPictures() {
    const url = "http://localhost:1337/api/pictures?populate=*";
    ...
    fetch(url, {
        ...
        .then(data => {
            const pictures = data.data;            // Clear content
            ...
            pictures.forEach(picture => {
                ...
                image.src = `http://localhost:1337${picture.attributes.name.data[0].attributes.url}`;
...
function fetchingsmallone() {
    const url = "http://localhost:1337/api/smallpictureones?populate=*";

    // Set loading state
    const divsmall = document.querySelector(".smallpictures");
    ...
    fetch(url, {
        ...
        .then(data => {
            ...
            pictures.forEach(picture => {
                ...
                images.classList.add("smallones");
                images.src = `http://localhost:1337${picture.attributes.image.data.attributes.url}`;
...
function fetchProducts() {
    ...
    const productContainer = document.querySelector(".sellssection"); // Corrected selector
