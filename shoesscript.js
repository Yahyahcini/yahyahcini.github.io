...
function fetchshoespictures() {
    const url = "http://localhost:1337/api/shoespictures?populate=*";
    ...
    fetch(url, {
        ...
        .then(data => {
            const shoespictures = data.data; 
            slides.innerHTML = ""; // Clear previous content

            // Create and display images for each parfumpicture
            shoespictures.forEach(shoespicture => {
                const image = document.createElement("img");
                image.classList.add("slide");
                image.src = `http://localhost:1337${shoespicture.attributes.photo.data.attributes.url}`;
...
function fetchProducts() {
    const url = "http://localhost:1337/api/shoesproducts?populate=*";
    const productContainer = document.querySelector(".shoesproductsection"); 
